...
class Cell(object):
    ...
    def __init__(self, x, y, func, screen, func_number):
        ...
        pygame.draw.rect(
            self.screen,
            (8, 22, 22),
            (self.x - 5, self.y - 5, self.WIDTH + 10, self.WIDTH + 10),
            3,
        )
    ...
    def update(self, event, buttons):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if (
                event.button == 1
                and x_mouse >= FILL_WIDTH
                and x_mouse <= self.x + self.WIDTH
                and self.y <= y_mouse
                and y_mouse <= self.y + self.WIDTH
                and self.x <= x_mouse
                and not self.draw
            ):  # левая кнопка мыши
                for b in buttons:
                    if b.draw and b != self:
                        pygame.draw.rect(
                            b.screen,
                            (8, 22, 22),
                            (b.x - 5, b.y - 5, b.WIDTH + 10, b.WIDTH + 10),
                            3,
                        )
                        b.draw = False
                self.draw = True
                ...
                and x_mouse < FILL_WIDTH
                and self.draw
                and (y_in_ar <= 11 or y_in_ar >= 19 or x_in_ar <= 8 or x_in_ar >= 19)
                and (y_in_ar >= 2 and y_in_ar <= 30)
                and (x_in_ar >= 1 and x_in_ar <= 26)
            ):
